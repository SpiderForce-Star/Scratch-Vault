# Store listing copy — Scratch Vault

Paste-ready for App Store Connect and Google Play.

| Field | Value |
|---|---|
| Category | **Reference** (or Magazines & Newspapers) |
| Do not use | Games, Casino, Lottery operator |
| Age | 18+ |
| Privacy | https://volunteer-scratch-vault.vercel.app/privacy |
| Terms | https://volunteer-scratch-vault.vercel.app/terms |

---

## Apple

**Name (30):** Scratch Vault  
_13 / 30_

**Subtitle (30):** Remaining-prize desk  
_21 / 30_

**Keywords (100, no spaces after commas):**

```
tennessee,scratch,off,remaining,prizes,instant,games,heat,desk,tn
```

_65 / 100_

**Promotional text (170):**

```
Review which scratch-offs still have prizes posted. Mid-tier heat. Bust list. Radar when the desk updates. 18+.
```

_121 / 170_

---

## Google Play

**Title (30):** Scratch Vault  
_13 / 30_

**Short description (80):**

```
TN scratch-off remaining prizes. Heat, mid-tier, skip list.
```

_59 / 80_

---

## Description (Apple + Google)

First three lines show before “more.” They do all the work:

See which TN instant games still have mid-tier cash posted.  
Skip drained games. Rank by heat, not hype.  
Independent desk. Not the Lottery. Remaining counts do not improve odds.

Scratch Vault is an independent remaining-prize desk for scratch-off games. It ranks publicly posted counts: what is still posted, what is drained, and what to skip. It does not sell tickets, take wagers, or pay prizes. It is not affiliated with, endorsed by, or connected to any state lottery.

Full Access is $4.99/month after a 7-day card-required trial, or $49.99/year billed now. No refunds. Opt out in Account. Store settings also apply. Radar alerts tell you when new remaining-prize information is on the desk, so you can review rankings before you buy a ticket. Informational only. Remaining counts do not improve your odds of winning.

You must be 18 or older. If gambling is no longer fun, call or text 1-800-GAMBLER (1-800-522-4700) or the Tennessee REDLINE at 1-800-889-9789.

Webb Spinner Visions · https://webbspinnervisions.net

---

## What’s New (1.0)

First release. Remaining-prize heat, skip list, and radar when the desk updates. Independent of every state lottery. 18+. 1-800-GAMBLER.

---

## Screenshot captions (large type on frames)

1. HOOK — See what’s still posted.  
2. SKIP — Skip the drained ones.  
3. DESK — Heat. Price. Week stamp.  
4. TRIP — Review these 3. Then stop.  
5. ALERT — New counts on the desk.  
6. TRUST — Independent. 18+. Then put the phone away.

Files: `store/ios/screenshots/` and `store/android/screenshots/`.

---

## Privacy nutrition (email + purchases only)

See earlier App Privacy / Data safety table in this folder’s prior notes. Do not add location, contacts, tracking, or ads.
